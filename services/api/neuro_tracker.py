...
def calculate_cognitive_affinity(user_profile, hackathon_tags: list[str]) -> float:
    """
    ...
    strengths = getattr(user_profile, 'cognitive_strengths', []) or []
    ...
    for tag in tags_lower:
        if tag in SKILL_COGNITIVE_MAP:
            ...
            if category and hasattr(category, 'value') and category.value in [str(s) for s in strengths]:
