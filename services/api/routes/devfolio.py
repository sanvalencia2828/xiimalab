...
def _make_cache_key(
    filters: dict, sort_by: str, limit: int, offset: int, wallet: str | None = None
) -> str:
...
# ─────────────────────────────────────────────
# Main endpoint — Phase 1 + Phase 3 combined
# ─────────────────────────────────────────────
...
async def list_devfolio_hackathons(
    ...
    sort_by: str = Query("match_score", description="Sort by: match_score, urgency, prize, deadline, personalized_score"),
    ...
    **Sort Options:**
    ...
    - `personalized_score`: Requires `wallet` param
    ...
    **Examples:**
    ...
    for h in hackathons:
        ...
        if wallet and user_profile:
            try:
                ...
                log.warning(f"Error computing personalized score for {h.id}: {e}")
    ...
    if wallet and user_profile and sort_by == "personalized_score":
        response_hackathons.sort(
            key=lambda h: getattr(h, "personalized_score", 0) or 0,
            reverse=True,
        )
